...
xs::particle_system init_particle_system(const size_t num_particles, float spacing, std::vector<size_t>& particle_idxs)
{
	const size_t sqrt_num_particles = std::sqrt(num_particles);
	particle_idxs.reserve(num_particles);
	...
	xs::particle_system particle_system = xs::particle_system(particles, .5f, .5f, .5f, .5f); // placeholders
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	xs::particle_system particle_system = init_particle_system(100, 0.01f, particle_idxs);

	using namespace std::placeholders;  // for _1, _2, _3...
	std::shared_ptr<xs::scene> scene = std::make_shared<xs::scene>();
	const xs::system_id eval_pressure_sid = scene->get_system_registry().register_system(
		std::bind(&xs::particle_system::evaluate_pressure, &particle_system, _1, _2, _3)
	);
	const xs::system_id eval_pressure_force_sid = scene->get_system_registry().register_system(
		std::bind(&xs::particle_system::evaluate_pressure_force, &particle_system, _1, _2, _3)
	);
	const xs::system_id eval_friction_force_sid = scene->get_system_registry().register_system(
		std::bind(&xs::particle_system::evaluate_friction_force, &particle_system, _1, _2, _3)
	);
	const xs::applicator_id particle_system_aid = scene->get_applicator_registry().register_system(
